...
export default function GuidesPageClient({ faqs }: { faqs: FaqEntry[] }) {
  const [openFaq, setOpenFaq] = useState<number | null>(faqs.length > 0 ? 0 : null);
            ...
            {[0, 1].map((col) => (
              <div className="guides-faq__col" key={col}>
                {faqs.map((faq, i) => {
                  ...
                  const panelId = `guides-faq-panel-${faq.id}`;
                  const triggerId = `guides-faq-trigger-${faq.id}`;
                  return (
                    <div key={faq.id} className={`guides-faq__item${isOpen ? " is-open" : ""}`}>
                      <h3 style={{ margin: 0 }}>
                        <button
                          id={triggerId}
                          type="button"
                          ...
                          onClick={() => toggleFaq(i)}
                        >
                          <span className="guides-faq__q-text">{faq.question}</span>
                      ...
                      <div id={panelId} role="region" aria-labelledby={triggerId} className="guides-faq__panel">
                        <div className="guides-faq__panel-inner" inert={!isOpen ? true : undefined}>
                          <p className="guides-faq__a">{faq.answer}</p>
                        </div>
                      </div>
                    </div>
                  );
